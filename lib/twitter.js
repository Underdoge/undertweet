...
exports.getTweetById = async function (id) {
    let htmlKeys = ['&amp;', '&lt;', '&gt;', '&#(\\d+);'],
        result = await needle("get", `https://api.twitter.com/2/tweets?ids=${id}&tweet.fields=public_metrics,author_id,context_annotations,source,referenced_tweets,created_at&expansions=referenced_tweets.id`, { headers: { "Accept": "application/json", "authorization": `Bearer ${token}`, "User-Agent": "v2TweetLookupJS"}});
