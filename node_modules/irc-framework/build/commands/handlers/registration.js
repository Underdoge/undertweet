...
var handlers = {
    RPL_WELCOME: function RPL_WELCOME(command) {
    ...
    RPL_ISUPPORT: function RPL_ISUPPORT(command) {
    ...
    CAP: function CAP(command) {
        ...
        var want = ['batch', 'multi-prefix', 'draft/message-tags-0.2', 'away-notify', 'invite-notify', 'account-notify', 'account-tag', 'server-time', 'userhost-in-names', 'extended-join', 'znc.in/server-time-iso', 'znc.in/server-time'];
        ...
        want = _(want).concat(this.request_extra_caps).uniq().value();
    ...
    AUTHENTICATE: function AUTHENTICATE(command) {
        var auth_str = this.connection.options.nick + '\0' + this.connection.options.nick + '\0' + this.connection.options.password;
    ...
    RPL_SASLAUTHENTICATED: function RPL_SASLAUTHENTICATED() {
        this.connection.write('CAP END');
        this.network.cap.negotiating = false;
    },

    RPL_SASLLOGGEDIN: function RPL_SASLLOGGEDIN() {
    ...
    ERR_SASLNOTAUTHORISED: function ERR_SASLNOTAUTHORISED() {
        this.connection.write('CAP END');
        this.network.cap.negotiating = false;
    },

    ERR_SASLABORTED: function ERR_SASLABORTED() {
        this.connection.write('CAP END');
        this.network.cap.negotiating = false;
    },

    ERR_SASLALREADYAUTHED: function ERR_SASLALREADYAUTHED() {
        // noop
    }
};
