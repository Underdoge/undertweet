...
var handlers = {
    NOTICE: function NOTICE(command) {
        ...
        if (message.charAt(0) === '\x01' && message.charAt(message.length - 1) === '\x01') {
            ...
            notice_from_server = command.prefix === this.network.server || !this.connection.registered;
    ...
    PRIVMSG: function PRIVMSG(command) {
        var time = command.getServerTime();
        var message = command.params[command.params.length - 1];
        var target = command.params[0];
        var target_group;
        ...
        if (message.charAt(0) === '\x01' && message.charAt(message.length - 1) === '\x01') {
            ...
            if (ctcp_command === 'ACTION') {
                ...
                this.connection.write(util.format('NOTICE %s :\x01VERSION %s\x01', command.nick, this.connection.options.version));
            } else if (ctcp_command === 'CLIENTINFO') {
                this.connection.write(util.format('NOTICE %s :\x01CLIENTINFO VERSION\x01', command.nick));
            } else {
    ...
    RPL_WALLOPS: function RPL_WALLOPS(command) {
        this.emit('wallops', {
            from_server: false,
            nick: command.nick,
            ident: command.ident,
