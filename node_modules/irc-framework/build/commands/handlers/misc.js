...
var handlers = {
    RPL_LISTSTART: function RPL_LISTSTART() {
        var cache = getChanListCache(this);
        cache.channels = [];
        this.emit('channel list start');
    },

    RPL_LISTEND: function RPL_LISTEND() {
        var cache = getChanListCache(this);
    ...
    RPL_LIST: function RPL_LIST(command) {
        var cache = getChanListCache(this);
        cache.channels.push({
    ...
    RPL_MOTD: function RPL_MOTD(command) {
        var cache = this.cache('motd');
        cache.motd += command.params[command.params.length - 1] + '\n';
    },

    RPL_MOTDSTART: function RPL_MOTDSTART() {
        var cache = this.cache('motd');
        cache.motd = '';
    },

    RPL_ENDOFMOTD: function RPL_ENDOFMOTD() {
        var cache = this.cache('motd');
        this.emit('motd', {
            motd: cache.motd
        });
    ...
    ERR_NOMOTD: function ERR_NOMOTD(command) {
    ...
    RPL_WHOREPLY: function RPL_WHOREPLY(command) {
        ...
        // G = Gone, H = Here
        var is_away = params[6][0].toUpperCase() === 'G' ? true : false;
        var hops_away = 0;
    ...
    RPL_ENDOFWHO: function RPL_ENDOFWHO(command) {
        var cache = this.cache('who');
        this.emit('wholist', {
    ...
    RPL_LINKS: function RPL_LINKS(command) {
    ...
    RPL_ENDOFLINKS: function RPL_ENDOFLINKS(command) {
        var cache = this.cache('links');
